      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {Array.isArray(projects) && projects.length > 0 ? (
          projects.map((project) => {
            const id = project.id;
            const pTitle = language === "en" ? t(`projectsData.${id}.title`) || project.title : project.title;
            const pShortDesc = language === "en" ? t(`projectsData.${id}.shortDesc`) || project.shortDesc : project.shortDesc;
            return (
              <CardProject
                id={id}
                title={pTitle}
          ...
          <div className="col-span-full text-center py-20">
            <h5 className="text-xl font-bold text-slate-400">{t("projects.noProjects")}</h5>
          </div>
        )}
      </div>
