      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10">
          <h1 className="text-4xl md:text-5xl font-bold text-slate-100 mb-4">
            {language === "en" ? t(`projectsData.${id}.title`) || project.title : project.title}
          </h1>
          <p className="text-xl text-slate-400 leading-relaxed">
            {language === "en" ? t(`projectsData.${id}.shortDesc`) || project.shortDesc : project.shortDesc}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-16">
          ...
          <div className="md:col-span-2 space-y-10">
            <div>
              <h3 className="text-2xl font-bold text-slate-200 mb-4">{t("projects.overview")}</h3>
              <p className="text-slate-400 leading-relaxed">
                {language === "en" ? t(`projectsData.${id}.fullDesc`) || project.fullDesc : project.fullDesc}
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800">
                <h4 className="font-bold text-slate-200 mb-2">{t("projects.challenge")}</h4>
                <p className="text-sm text-slate-400">
                  {language === "en" ? t(`projectsData.${id}.challenge`) || project.challenge : project.challenge}
                </p>
              </div>
              <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800">
                <h4 className="font-bold text-slate-200 mb-2">{t("projects.solution")}</h4>
                <p className="text-sm text-slate-400">
                  {language === "en" ? t(`projectsData.${id}.solution`) || project.solution : project.solution}
                </p>
              </div>
            </div>

            <div>
              <h3 className="text-2xl font-bold text-slate-200 mb-4">{t("projects.keyResults")}</h3>
              <div className="flex items-start gap-3">
                <FaCircleCheck className="text-green-500 mt-1 text-xl shrink-0" />
                <p className="text-slate-400 leading-relaxed">
                  {language === "en" ? t(`projectsData.${id}.result`) || project.result : project.result}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-800">
              <h3 className="text-lg font-bold text-slate-200 mb-4">{t("projects.technologies")}</h3>
            ...
            {/* Links */}
            <div className="bg-slate-900/30 p-6 rounded-2xl border border-slate-800">
              <h3 className="text-lg font-bold text-slate-200 mb-4">{t("projects.projectLinks")}</h3>
